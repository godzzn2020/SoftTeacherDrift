#!/usr/bin/env python
"""Track G：两阶段触发（candidate OR → confirm weighted）对照评估。"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Track G：OR vs weighted vs two_stage")
    ...
    parser.add_argument("--theta_weighted", type=float, default=0.5, help="weighted 的阈值")
    parser.add_argument("--confirm_window", type=int, default=200)
    parser.add_argument("--theta_confirm", type=float, default=0.5)
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--logs_root", type=str, default="logs")
    parser.add_argument("--results_root", type=str, default="results")
    parser.add_argument("--run_name", type=str, default="trackG")
    parser.add_argument("--out_csv", type=str, default="scripts/TRACKG_TWO_STAGE.csv")
...
def load_synth_meta(synth_root: Path, dataset: str, seed: int) -> Tuple[List[int], int]:
    ...
    obj = __import__("json").loads(meta_path.read_text(encoding="utf-8"))
        ...
        if r.get("drift_flag") == "1":
            ...
            if x:
                try:
                    ...
                    pass
            d = r.get("confirm_delay")
            if d is not None and d != "" and d != "-1":
                try:
                    delays.append(int(float(d)))
                except ValueError:
                    pass
    return {
...
def main() -> int:
    ...
    variants = [
        ("or", {"trigger_mode": "or", "trigger_threshold": 0.5, "confirm_window": int(args.confirm_window)}),
        ("weighted", {"trigger_mode": "weighted", "trigger_threshold": float(args.theta_weighted), "confirm_window": int(args.confirm_window)}),
        ("two_stage", {"trigger_mode": "two_stage", "trigger_threshold": float(args.theta_confirm), "confirm_window": int(args.confirm_window)}),
    ]

    records: List[Dict[str, object]] = []
    for tag, mode_cfg in variants:
        experiment_run: ExperimentRun = create_experiment_run(
            experiment_name="trackG_two_stage",
        ...
        for dataset in datasets:
            for seed in seeds:
                cfg_map = build_cfg_map(seed=seed)
                ...
                cfg = replace(
                    ...
                    trigger_threshold=float(mode_cfg["trigger_threshold"]),
                    trigger_k=2,
                ...
                gt, horizon = load_synth_meta(synth_root, dataset, seed)
                stats = read_log_metrics(log_path)
                dets = stats["detections"]  # type: ignore[assignment]
                win = compute_detection_metrics(gt, dets, int(horizon)) if horizon > 0 else {"MDR": math.nan, "MTD": math.nan, "MTFA": math.nan, "MTR": math.nan}
                dets_merged = _merge_events(dets, int(args.min_separation))
                tol = compute_metrics_tolerance(gt, dets_merged, int(horizon), int(args.match_tolerance))

                cands = stats["candidates"]  # type: ignore[assignment]
                cands_merged = _merge_events(cands, int(args.min_separation))
                delays = stats["confirm_delays"]  # type: ignore[assignment]
                ...
                records.append(
                    {
                        ...
                        "theta": float(mode_cfg["trigger_threshold"]),
                        "confirm_window": int(mode_cfg["confirm_window"]),
                        "acc_final": stats["acc_final"],
                        "mean_acc": stats["mean_acc"],
                        "acc_min": stats["acc_min"],
                        "candidate_count": len(cands_merged),
                        "confirmed_count": len(dets_merged),
                        "confirm_delay_mean": _mean([float(x) for x in delays]) if delays else None,
                        "confirm_delay_std": _std([float(x) for x in delays]) if delays else None,
                        "MDR_win": win.get("MDR"),
                        "MTD_win": win.get("MTD"),
