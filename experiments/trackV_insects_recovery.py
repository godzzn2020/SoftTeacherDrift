#!/usr/bin/env python
"""
NEXT_STAGE V8 - Track V：INSECTS 恢复机制定稿（创新点2）

dataset = INSECTS_abrupt_balanced
seeds = 1..20（可通过 --seeds 调整）

检测侧固定主方法：two_stage + tuned PH + cooldown=200
...
对比组：
A) baseline（无 severity v2）
B) severity v2
C) severity v2 + confirmed drift gating（v2_gate_m5）

输出（run 粒度）：scripts/TRACKV_INSECTS_RECOVERY.csv
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Track V: INSECTS recovery (v2/gating final)")
    ...
    p.add_argument("--out_csv", type=str, default="scripts/TRACKV_INSECTS_RECOVERY.csv")
...
def ensure_log(
    ...
    *,
    ...
    cfg = replace(
        ...
        # 对齐既有 severity 实验（Track H/N/L）的常用设置，减少“默认值漂移”带来的不可比。
...
def main() -> int:
    ...
    for g in groups:
        ...
        exp_run = create_experiment_run(
            experiment_name="trackV_insects_recovery",
