...
"""
V15.3 稳定性复核（tiny ablation）：
- 固定 3 个候选 + baseline（与 V15.2 对齐）
- 对每个候选仅新增 1 个变体：perm_test + perm_side=two_sided
- 修复 stagger_abrupt3 的 GT drift anchors（避免 default_gt_starts 产生“虚假漂移点”）
...
def main() -> int:
    ...
    for alpha in (0.02, 0.025, 0.03):
        ...
        # 变体：two-sided（方向不敏感）
        name2 = f"{name}_side2s"
        tw2 = dict(tw)
        tw2["__perm_side"] = "two_sided"
        groups.append(
            {
                "group": name2,
                "confirm_rule": "perm_test",
        ...
        for ds_alias, base_name, base_cfg, stream_kwargs, ds_kind in datasets:
            ...
            for seed in seeds:
                ...
                if ds_kind == "nodrift":
                    ...
                    if str(ds_alias).lower() == "stagger_abrupt3":
                        # stagger_abrupt3: segment_length=20000，真实漂移点仅在 20000/40000（horizon=51200 时为 2 次漂移）。
                        # 若误用 default_gt_starts(n_samples=50000) 会引入 10000/30000 的“虚假漂移点”，导致 miss/confP90 伪劣化。
            ...
            agg: Dict[str, Any] = {
                ...
                "group": str(g["group"]),
                "monitor_preset": str(args.monitor_preset),
                "signal_set": per_seed_records[0].get("signal_set") if per_seed_records else (args.signal_set or ""),
                "confirm_theta": float(args.confirm_theta),
                "confirm_window": int(args.confirm_window),
                "confirm_cooldown": int(args.confirm_cooldown),
                "confirm_rule": str(g["confirm_rule"]),
