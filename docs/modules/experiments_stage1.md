...
- `experiments/first_stage_experiments.py`：
  ...
  - CLI 支持批量运行 `--datasets`、`--models` 参数，自动保存日志到 `artifacts/legacy/logs/{dataset}/...`（历史产物归档目录）。
...
- `experiments/offline_detector_sweep.py`：
  - 读取已有 `artifacts/legacy/logs/{dataset}/{dataset}__{model}__seed{seed}.csv` 与对应 meta，离线重放多组 drift detector 组合。
  - 内置 `DetectorConfig` 网格（error/entropy/divergence × ADWIN/PageHinkley），输出 `artifacts/legacy/results/offline_detector_grid.csv` 与 per-dataset Markdown Top-N。
...
- `experiments/summarize_online_results.py`：
  - 汇总在线训练日志（`artifacts/legacy/logs/`）并生成 `artifacts/legacy/results/online_runs.csv`、`artifacts/legacy/results/online_summary.csv`、`artifacts/legacy/results/online_summary.md`。
  - 自带轻量级 SVG 绘图，将每个数据集的 Accuracy 曲线输出到 `artifacts/legacy/figures/online_accuracy/`，并在 `artifacts/legacy/figures/online_detections/` 绘制检测时间线；同时按日志中的 `drift_flag` 计算在线 MDR/MTD/MTFA/MTR，方便与离线 detector 结果对照。
- `evaluation/phaseA_signal_drift_analysis_synth.py`：
  - 针对合成流日志的信号层面对齐分析脚本，会读取 `artifacts/legacy/logs/` 中的 `student_error_rate`、`teacher_entropy`、`divergence_js` 与 `data/synthetic/..._meta.json` 的真值漂移。
...
- `evaluation/phaseC_scheduler_ablation_synth.py`：
  - 针对 `artifacts/legacy/logs/{dataset}__{model_variant}__seed{seed}.csv` 与 `data/synthetic/..._meta.json`，计算 `mean_acc`、`final_acc` 以及每个漂移窗口的 `drop_min_acc`，对比 `ts_drift_adapt` 与 `ts_drift_adapt_severity`。
...
- `experiments/stage1_multi_seed.py`：
  ...
  - 运行结束后调用 online summarizer 逻辑收集指标，输出 `artifacts/legacy/results/stage1_multi_seed_raw.csv`（逐 seed）、`artifacts/legacy/results/stage1_multi_seed_summary.csv`（按 dataset+model 的 mean/std），以及 `artifacts/legacy/results/stage1_multi_seed_md/{dataset}_multi_seed_summary.md`（Markdown 表）。
  - 示例命令：
    ```bash
