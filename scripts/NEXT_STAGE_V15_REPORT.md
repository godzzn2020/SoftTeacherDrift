...
- 产物：`scripts/TRACKAL_PERM_CONFIRM_SWEEP_V15.csv`
...
- - Track AM：已生成 `scripts/TRACKAM_PERM_DIAG_V15.csv` rows=8（p-value 分布/confirmed-candidate 比例见诊断表）
...
## V15 验收回答
- 生成时间：2026-01-12 01:38:40
- V14 winner 基线 no-drift（来自 `scripts/NEXT_STAGE_V14_REPORT.md`）：26.880
- 本次 V15 重跑基线（`A_weighted_n5`）：confirm_rate_per_10k=26.540
- 存在 perm_test 同时满足硬约束且降低 no-drift：`P_perm_vote_score_a0.02_pre500_post20_n5`（no_drift_rate=24.920；违约检查：N/A）
